...
fun ManageCakeScreen(navController: NavController) {
    ...
    val filteredCakes = if (searchQuery.isEmpty()) allCakes else { allCakes.filter { it.cakeName.contains(searchQuery, ignoreCase = true) } }
                ...
                actions = {
                    ...
                    if (allCakes.isNotEmpty()) {
                        IconButton(onClick = { showDeleteAllConfirmation = true }) {
                            Icon(Icons.Default.DeleteSweep, contentDescription = "Delete All Cakes")
                        }
                    }
                }
            )
